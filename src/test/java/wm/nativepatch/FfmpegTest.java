...
public final class FfmpegTest {
    ...
    public static void main(final String... args) throws Exception {
        ...
        final boolean ready1 = NativePatchLoader.deployFfmpeg(ffmpegDir);
        System.out.println("ready = " + ready1 + ", isBionic = " + NativePatchLoader.isBionic());
        ...
        assertTrue(!NativePatchLoader.ffmpegReady(ffmpegDir), "readiness detects foreign libjniavutil");

        System.out.println("== re-deploy fixes it ==");
        assertTrue(NativePatchLoader.deployFfmpeg(ffmpegDir), "re-deploy restores readiness");

        System.out.println("== preload (graceful on non-bionic host) ==");
        NativePatchLoader.preloadFfmpegCores(ffmpegDir); // must not throw
