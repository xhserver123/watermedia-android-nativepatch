...
javac --release 17 -encoding UTF-8 -Xlint:all \
    ...
    -d "$CLASSES" \
    src/main/java/wm/nativepatch/NativePatchLoader.java
...
cp src/main/resources/fabric.mod.json "$RES/fabric.mod.json"

echo "==> packaging"
( cd "$CLASSES" && jar cf "$OLDPWD/$OUT/watermedia-nativepatch-$VERSION.jar" . )
( cd "$RES" && jar uf "$OLDPWD/$OUT/watermedia-nativepatch-$VERSION.jar" . )

echo "==> verifying"
jar -tvf "$OUT/watermedia-nativepatch-$VERSION.jar" | tail -3
echo ""
echo "OK -> $OUT/watermedia-nativepatch-$VERSION.jar ($(du -h "$OUT/watermedia-nativepatch-$VERSION.jar" | cut -f1))"
